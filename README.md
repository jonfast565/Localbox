...
- **Batching / framing (wire v5):** Materialized intents become `BatchManifest` batches; `FileChunk` carries `batch_id` + `protocol_version`; `BatchAck` may include `batch_id` so the sender resolves the DB-local intent. Intent ids never go on the wire. Chunk payloads stream from disk on send and stage to a temp file on receive (hash-verified, then atomic rename). Hello/discovery also carry `display_name`, `app_state`, and structured share ads (`name` + `recursive`/`sync`/`pull`).
...
- **Chat:** Peer DM thread ids are deterministic (`min(local,remote)`); share threads key off share name. Messages persist in SQLite (`chat_threads` / `chat_messages`). Peer-thread titles prefer the peer's advertised `display_name`.
- **Discovery:** Nodes broadcast `DISCOVER` and respond with `HERE`, including display name, app state, and structured share tokens (`name:recursive:sync:pull`). Wire protocol version is 5; there is no version negotiation, so all peers must be upgraded together.
