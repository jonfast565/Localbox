...
fn export_ca_from_chain_and_import_dedupes() {
    ...
    let materials = peering::tls::generate_tls_materials("pc-test").unwrap();
    ...
    localbox::tls_workflow::export_ca_from_chain_pem(&chain_path, &exported_ca).unwrap();
    ...
    let added1 =
        localbox::tls_workflow::import_ca_into_trust_store(&trust_path, &exported_ca).unwrap();
    let added2 =
        localbox::tls_workflow::import_ca_into_trust_store(&trust_path, &exported_ca).unwrap();
    assert_eq!(added1, 1);
    assert_eq!(added2, 0);

    let fps = localbox::tls_workflow::read_trust_store_fingerprints(&trust_path).unwrap();
    assert_eq!(fps.len(), 1);
...
fn normalize_fingerprint_ignores_separators() {
    let a = localbox::tls_workflow::normalize_fingerprint("aa:bb:cc");
    let b = localbox::tls_workflow::normalize_fingerprint("AABBCC");
    assert_eq!(a, b);
}
